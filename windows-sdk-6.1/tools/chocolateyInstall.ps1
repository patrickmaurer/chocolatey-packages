try {
	...
	Start-Process "$toolsDir\7z.exe" -ArgumentList "x -o`"$toolsDir\KRMSDK_EN`" -y `"$toolsDir\KRMSDK_EN.iso`"" -Wait -WindowStyle Hidden -PassThru
	...
	Write-ChocolateyFailure 'windows-sdk-6.1' "$($_.Exception.Message)"
	throw
}